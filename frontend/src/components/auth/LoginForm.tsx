...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { riskScore, requiresStepUp, passwordExpired } = await login(
        email,
        password,
      );
      ...
      if (requiresStepUp) {
        toast(
          "⚠️ Unusual login detected. Additional verification may be required.",
          {
            icon: "🔒",
            duration: 5000,
          },
      ...
      if (riskScore > 30) {
        toast(
          `Risk Score: ${riskScore}/100 — Session under enhanced monitoring`,
          {
            icon: "🛡️",
          },
        );
